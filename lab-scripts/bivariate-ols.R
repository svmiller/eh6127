...
#' called `x` and `e` from a normal distribution.
#' 
...
#' `x` is going to be our independent variable, which is exogenously created 
#' prior to an observed outcome. `e` is going to be our error term, which 
#' includes everything not included in the linear model. The presence of an error 
#' term is an important assumption in OLS, and notice here that the error term is 
#' created *independently from the x variable*. Yes, they're in the same data set, 
...
#' Notice that the correlation between *x* and *e* is about .06, but the 95% 
#' interval includes 0. The modest correlation we do observation is indeed due to 
#' random chance.
#' 
#' Now, watch this. We're going to create a new variable, *y*, that is a linear 
#' function of *x* and *e*, plus some baseline value (intercept) when *x* is 0.
#' 
Fake %>%
  mutate(y = 5 + x + e) -> Fake
...
#' simulating data to evaluate a model, you're playing god to see how well the 
#' model can capture the "true" parameters you're generating. The "true" value 
#' of *y* is 5 when *x* is 0. The "true" effect of *x* on *y* is to increase *y* 
#' by 1 for every one-unit increase in *x*. The error term here has equal footing 
#' to *x*, but recall that the error term (*e*) includes everything else that 
#' influences *y* but is not included in the model. It's ultimately going to 
#' introduce noise into how well we can capture the other true parameters.
#' 
#' The data are fake, sure, but the process of simulating so-called "random 
#' variables" (drawn from a normal distribution with pre-defined parameters) is 
#' useful for illustrating the properties of a statistical model like this. It'll 
#' also help to show what these data "look like." Here, for example, is the 
#' distribution of *x*. When you have granularity in a variable like this 
#' (with so many trailing decimals), it's useful to jettison a bar chart or 
#' histogram (which has somewhat arbitrary bin width) in favor of the density 
#' plot. The density plot may not have the same easy takeaway as to what's 
#' happening on the *y*-axis, but it's arguably more useful for getting the 
#' basic shape of a distribution.
#' 
#' There are two ways of doing this. Here's the base R way.
#' 
plot(density(Fake$x))
#'
#' Here, we see the data look nice and "normal" with a clear central tendency 
#' and a shape approximating a true bell curve.
#' 
#' Here's the prettier ggplot way. ggplot stuff (by way of `{ggplot2}`) comes 
#' default in `{tidyverse}`. Once you load `{tidyverse}`, you load all the 
#' `{ggplot2}` functionality you could want.
#' 
Fake %>%
  ggplot(.,aes(x)) +
  geom_density() 
...
#' endogenous part of our data set. It is an outcome of two exogenous variables 
#' (*x*, which we can measure, and *e*, which serves as a plug-in for background 
#' noise in the underlying population).
#' 
#' Can OLS capture these known attributes about the data? We'll use the `lm()` 
#' function in R to find out. `lm()` is the base R function for estimating 
#' linear models. It's syntax is simple, especially in a simple case like this. 
#' The two basic things you'll want to feed it are the formula, separated by a 
#' comma, then the name of the data set for which you want to regress your 
#' dependent variable on your independent variable(s). Here, we're doing a super 
#' simple regression of *y* on *x*. Then, we'll get a summary
#' of the model with `summary()`.
#' 
M1 <- lm(y ~ x, data=Fake)
summary(M1)

#' Our question was whether OLS can capture the known population parameters that 
#' we created. The answer is yes. Yes, it can. In a simple bivariate case, you 
#' can see what this would look like visually with a scatterplot. I want you to 
#' see what this is doing. First, we're going to use our data and, with the pipe 
#' operator (`%>%`), we're going to pass the data to a `ggplot()` function with 
#' a simple aesthetic (`aes()`) argument creating an x-axis for `x` and `y` axis 
#' for `y`. Then, using `+` (which is the `{ggplot2}` continuation operator),
#' we're going to create a scatterplot with `geom_point()`. Then, using another 
#' continuation operator (`+`), we're going to use `geom_smooth()` to create a 
#' line of best fit for the data. Importantly, you have to  specify you want a 
#' linear smoother (`method = "lm"`) to communicate what a simple bivariate case 
#' is doing.  The default is `method = "loess"`, which has its own nice features 
#' but it's not what we want here. Finally, we're going to create two lines over 
#' the plot. The first uses `geom_vline(xintercept = mean(Fake$x))` to add a 
#' vertical line through the mean of `x`. The second uses 
#' `geom_vline(yintercept = mean(Fake$y))` to create a horizontal line through 
#' the mean of `y`. Notice that the line of best fit runs through the 
#' intersection of both them.
...
#' 
#' This graph is great for visualizing a super simple bivariate case, but things 
#' are never so simple. So, let's  walk through a little bit of the important 
#' output from the model on its own terms. Let's start with the "coefficients",
#' since they are the main reason you're doing this. We mainly wanted to know if 
#' we could capture the "true" effect of *x* on *y*. The "true" effect is 1. The 
#' OLS estimator says it's about 1.06. That's technically wrong, sure, but the 
#' coefficient is what it is because it's also capturing the correlation between 
#' *x* and the error term *e*, which we *don't* get to include in the model. So, 
#' OLS says it's 1.06, which comes with a standard error of about .10. A standard 
#' error below the estimate is about .961. A standard error above the estimate is 
#' about 1.175.
#' 
#' Put another way, given estimation and fundamental uncertainty about the model, 
#' our estimated effect of *x* on *y* is within a standard error of what we know 
#' it actually is. We captured the true population parameter of interest to us.
#' 
#' The other thing you want to know from your regression model is whether the 
#' effect of your independent variable of interest on the dependent variable is 
#' "statistically significant." This term is very misleading, to be frank. About 
#' 10 years ago, it appeared on this list of [scientific ideas that 
